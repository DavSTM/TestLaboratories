...
@doc_acknowledgement_ack_bp.route(
    "/document_acknowledgement_acknowledge", methods=["POST"]
)
def document_change_agreed():
    ...
    (
        ...
        base_fields,
        user_email,
    ) = get_records_by_transfer_id("Документы - Ознакомление", booSame=True)

    person_confirm(
        "Документы - Ознакомление",
        temp_record_id,
        base_record_id,
        base_record,
        "Ознакомлены - ID",
