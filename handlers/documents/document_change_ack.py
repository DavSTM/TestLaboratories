...
doc_change_ack_bp = Blueprint("document_change_ack", __name__)


@doc_change_ack_bp.route("/document_change_acknowledge", methods=["POST"])
def document_change_acknowledge():
    data = request.json
    ...
    temp_record = get_record("Документы - Изменения", temp_id)
    if not temp_record:
    ...
    acknowledger_ids = base_fields.get("Согласовано ID", [])
